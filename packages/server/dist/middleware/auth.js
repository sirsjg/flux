 ...
 */
export function canWriteProject(auth, projectId) {
    if (!isAuthRequired())
        return true;
 ...
 */
export function canReadProject(auth, projectId) {
 ...
 */
export function filterProjects(auth) {
    ...
    if (auth.keyType === 'project' && auth.projectIds) {
        return projects.filter(p => auth.projectIds.includes(p.id) || p.visibility !== 'private');
    }
 ...
 */
export function hasServerAccess(auth) {
    if (!isAuthRequired())
        return true;
 ...
 */
export const requireServerAccess = createMiddleware(async (c, next) => {
