...
import { setStorageAdapter, initStore, resetStore, getProject, createProject, updateProject, deleteProject, getProjectStats, getEpics, getEpic, createEpic, updateEpic, deleteEpic, getTasks, getTask, createTask, updateTask, deleteTask, addTaskComment, deleteTaskComment, isTaskBlocked, getReadyTasks, cleanupProject, getWebhooks, getWebhook, createWebhook, updateWebhook, deleteWebhook, getWebhookDeliveries, setWebhookEventHandler, triggerWebhooks, getApiKeys, createApiKey, deleteApiKey, createCliAuthRequest, completeCliAuthRequest, pollCliAuthRequest, cleanupExpiredAuthRequests, setAuthFunctions, createBlob, getBlob as getStoreBlob, getBlobs as getStoreBlobs, deleteBlob as deleteStoreBlob, } from '@flux/shared';
...
import { authMiddleware, filterProjects, canReadProject, canWriteProject, requireServerAccess } from './middleware/auth.js';
...
function validateTaskFields(body) {
    if (body.acceptance_criteria !== undefined) {
        ...
        if (body.acceptance_criteria.some((c) => typeof c !== 'string' || !c.trim())) {
...
app.get('/api/projects/:projectId/tasks', (c) => {
    ...
    const statusFilter = c.req.query("status"); const epicFilter = c.req.query("epic_id"); let filteredTasks = getTasks(projectId); if (statusFilter) { filteredTasks = filteredTasks.filter(t => t.status === statusFilter); } if (epicFilter) { filteredTasks = filteredTasks.filter(t => t.epic_id === epicFilter); } const tasks = filteredTasks.map(t => ({
        ...t,
...
app.get('/api/blobs/:id/content', (c) => {
    ...
    if (!content)
        return c.json({ error: 'Blob content not found' }, 404);
...
if (config.sync?.enabled) {
    // Persist sync changelog alongside the data file so it survives restarts
