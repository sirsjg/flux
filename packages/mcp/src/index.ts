...
import { setStorageAdapter, initStore, STATUSES, WEBHOOK_EVENT_TYPES, type Guardrail, type Priority } from '@flux/shared';
        ...
        inputSchema: {
          ...
          properties: {
            ...
            priority: {
              type: 'integer',
              enum: [0, 1, 2],
              description: 'Task priority: 0=P0 (urgent), 1=P1 (normal), 2=P2 (low). Defaults to 1 if not provided.',
            },
            type: {
              type: 'string',
              enum: ['task', 'bug', 'feature', 'refactor', 'docs', 'chore'],
              description: 'Task type: task, bug, feature, refactor, docs, chore. Defaults to "task" if not provided.',
            },
            acceptance_criteria: {
              type: 'array',
              items: { type: 'string' },
        ...
        inputSchema: {
          ...
          properties: {
            ...
            priority: {
              type: 'integer',
              enum: [0, 1, 2],
              description: 'Task priority: 0=P0 (urgent), 1=P1 (normal), 2=P2 (low)',
            },
            type: {
              type: 'string',
              enum: ['task', 'bug', 'feature', 'refactor', 'docs', 'chore'],
              description: 'Task type: task, bug, feature, refactor, docs, chore',
            },
            depends_on: {
              type: 'array',
              items: { type: 'string' },
