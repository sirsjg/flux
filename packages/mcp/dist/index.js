...
import { initClient, getProjects, getProject, createProject, updateProject, deleteProject, getProjectStats, getEpics, getEpic, createEpic, updateEpic, deleteEpic, getTasks, getTask, createTask, updateTask, deleteTask, addTaskComment, deleteTaskComment, isTaskBlocked, getReadyTasks, getWebhooks, createWebhook, updateWebhook, deleteWebhook, getWebhookDeliveries, uploadBlob, downloadBlob, getClientBlobs, deleteBlobClient, } from '@flux/shared/client';
...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
    ...
    if (uri === 'flux://projects') {
        ...
        const projects = await Promise.all(projectList.map(async (p) => ({
            ...p,
    ...
    if (tasksMatch) {
        ...
        const tasks = await Promise.all(taskList.map(async (t) => ({
            ...t,
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    switch (name) {
        ...
        case 'update_project': {
            ...
            const project = await updateProject(args?.project_id, updates);
            if (!project) {
        ...
        // Epic operations
        case 'list_epics': {
            const epics = await getEpics(args?.project_id);
            return {
                content: [{ type: 'text', text: JSON.stringify(epics, null, 2) }],
            };
        }
        case 'create_epic': {
            const epic = await createEpic(args?.project_id, args?.title, args?.notes, args?.auto);
            return {
        ...
        case 'update_epic': {
            ...
            if (args?.auto !== undefined)
                updates.auto = args.auto;
            const epic = await updateEpic(args?.epic_id, updates);
            if (!epic) {
        ...
        case 'delete_epic': {
            const success = await deleteEpic(args?.epic_id);
            if (!success) {
                return { content: [{ type: 'text', text: 'Epic not found' }], isError: true };
            }
            return {
        ...
        // Task operations
        case 'list_tasks': {
            const taskList = await getTasks(args?.project_id);
            let tasks = await Promise.all(taskList.map(async (t) => ({
                ...t,
                blocked: await isTaskBlocked(t.id),
            })));
            // Apply filters
        ...
        case 'list_ready_tasks': {
            const tasks = await getReadyTasks(args?.project_id);
            return {
                content: [{ type: 'text', text: JSON.stringify(tasks, null, 2) }],
            };
        }
        case 'create_task': {
            const task = await createTask(args?.project_id, args?.title, args?.epic_id, {
                acceptance_criteria: args?.acceptance_criteria,
                guardrails: args?.guardrails, // IDs generated by store if missing
            });
            return {
        ...
        case 'update_task': {
            ...
            if (args?.status === 'in_progress') {
                const currentTask = await getTask(args?.task_id);
            ...
            if (args?.epic_id !== undefined)
                updates.epic_id = args.epic_id || undefined;
            if (args?.depends_on)
                updates.depends_on = args.depends_on;
            ...
            if (args?.guardrails !== undefined)
                updates.guardrails = args.guardrails;
        ...
        case 'delete_task': {
            const success = await deleteTask(args?.task_id);
            if (!success) {
                return { content: [{ type: 'text', text: 'Task not found' }], isError: true };
            }
            return {
        ...
        case 'move_task_status': {
            ...
            const statusUpdates = { status: args?.status };
            ...
            const task = await updateTask(args?.task_id, statusUpdates);
            if (!task) {
                return { content: [{ type: 'text', text: 'Task not found' }], isError: true };
            }
            return {
        ...
        case 'add_task_comment': {
            ...
            const agentName = args?.agent_name;
            const comment = await addTaskComment(args?.task_id, body, author, agentName);
            if (!comment) {
                return { content: [{ type: 'text', text: 'Task not found' }], isError: true };
            }
            return {
        ...
        case 'delete_task_comment': {
            const success = await deleteTaskComment(args?.task_id, args?.comment_id);
            if (!success) {
        ...
        case 'get_epic_progress': {
            ...
            const epicTasks = tasks.filter((t) => t.epic_id === epicId);
            const byStatus = { planning: 0, todo: 0, in_progress: 0, done: 0 };
            const activeWorkers = [];
        ...
        case 'get_agent_activity': {
            const agentFilter = args?.agent_name;
            const projects = await getProjects();
            ...
            for (const project of projects) {
                ...
                for (const task of tasks) {
                    ...
                    if (task.status === 'done' && task.completed_by) {
                        if (agentFilter && task.completed_by !== agentFilter)
                            continue;
        ...
        case 'get_project_overview': {
            ...
            for (const t of allTasks) {
                if (t.status === 'in_progress' && t.workers) {
                    for (const w of t.workers) {
                        if (!agentTasks.has(w))
                            agentTasks.set(w, { count: 0, epics: new Set() });
                        const data = agentTasks.get(w);
                        data.count++;
                        if (t.epic_id)
                            data.epics.add(t.epic_id);
                    }
                }
            }
            ...
            const recentlyCompletedCount = allTasks.filter((t) => t.status === 'done' && t.completed_at && t.completed_at > oneDayAgo).length;
        ...
        case 'create_webhook': {
            const webhook = await createWebhook(args?.name, args?.url, args?.events, {
                secret: args?.secret,
        ...
        case 'update_webhook': {
            ...
            if (args?.secret !== undefined)
                updates.secret = args.secret || undefined;
            if (args?.project_id !== undefined)
                updates.project_id = args.project_id || undefined;
            if (args?.enabled !== undefined)
                updates.enabled = args.enabled;
            const webhook = await updateWebhook(args?.webhook_id, updates);
            if (!webhook) {
        ...
        case 'delete_webhook': {
            const success = await deleteWebhook(args?.webhook_id);
            if (!success) {
                return { content: [{ type: 'text', text: 'Webhook not found' }], isError: true };
            }
            return {
        ...
        case 'list_webhook_deliveries': {
            ...
            const deliveries = await getWebhookDeliveries(args?.webhook_id, limit);
            return {
